...
/**
 * Created by irtazasafi on 11/25/16.
 */
public class CYK_PARALLEL_THREE {
    ...
    public static int NUM_INTRA_CELL_THREADS = 2;

    public static ExecutorService executors = Executors.newFixedThreadPool(NUM_INTRA_CELL_THREADS);

    public static boolean allFuturesDone(ArrayList<Future<tableItem>> scheduledJobs) {
        for(Future<tableItem> job:scheduledJobs){
            if(!job.isDone()) {
                return false;
            }
        }
    ...
    public static tableItem computeForSlot(int x,int y) throws InterruptedException, ExecutionException {
        // collect all items going down and diagonal from this slot
        ArrayList<tableItem> forUnion = new ArrayList<tableItem>();
        ArrayList<Future<tableItem>> scheduledJobs = new ArrayList<>();
        int downStartX = x;
        int downStartY = 0;
        ...
        for(int steps = 0; steps < y;steps++){
            ...
            scheduledJobs.add(executors.submit(() -> table.multiply(downItem,diagonalItem,x,y)));
            downStartY+=1;
            diagonalStartX+=1;
            diagonalStartY-=1;
        }
        // wait for all future completion (Just in case)

        while(!allFuturesDone(scheduledJobs)){};

        for(Future<tableItem> completedJob: scheduledJobs){
            forUnion.add(completedJob.get());
        }
...
//        });
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("RUNNING CYK ORDERED JOBS and INTER CELL ALGORITHM");
